...
export class NLURouter extends CustomStudioRouter {
  ...
  setupRoutes() {
    ...
    this.router.post(
      ...
      this.needPermissions('write', 'bot.training'),
      this.asyncMiddleware(async (req, res) => {
        const { botId, lang, speed } = req.params
        try {
          const modelId = await this.nluService.lint(botId, lang, speed as IssueComputationSpeed)
          res.send(modelId)
        } catch (error) {
    ...
    this.router.get(
      ...
      this.needPermissions('read', 'bot.training'),
      this.asyncMiddleware(async (req, res) => {
        const { botId, modelId, speed } = req.params
        try {
          const linting = await this.nluService.getLinting(botId, modelId, speed as IssueComputationSpeed)
          res.send(linting)
        } catch (error) {
          return this._mapError({ botId, error }, res)
        }
